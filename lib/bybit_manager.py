"""Bybit market-data manager and 5-minute candle tracker.

This is the core of the bot's signal generation:

  - BybitCandle5m   — per-symbol 5m candle + outcome resolution. On bar close,
    walks the Redis history (prices:signals, prices:signals_raw,
    orders:pending_outcome) and writes the consensus direction across the
    four feeds so the dashboard can compute hit-rate per source.

  - BybitManager    — owns the Bybit WebSocket connections (inverse for
    ticker/orderbook, linear for liquidations), maintains TickData /
    OBI / volume / liquidation state, fires execute_trading_validation
    when a 5m bar pct exceeds the dynamic threshold, and produces the
    final go/no-go signal via get_signal() using N-of-M alignment across
    Bybit, Binance, and Coinbase.

Dependency injection
--------------------
Several runtime collaborators live in main.py and would create circular
imports if pulled directly. Inject them after construction:

  mgr = BybitManager(chainlink_feed=..., coinbase_feed=..., binance_feed=...)
  mgr.attach_components(rdb=..., finder=..., client=..., price_tracker=...)
  mgr.attach_validator(execute_trading_validation)
  mgr.start_websocket(loop)

The same late-binding pattern is used by BinanceFeed.attach_validator.
"""
...
class BybitCandle5m:
    ...
    `manager` is a back-reference to the BybitManager that owns this candle,
    used by `_update_order_outcomes` to read the live coinbase/binance/chainlink
    feeds without referencing main.py module globals.
    """

    def __init__(self, manager: "BybitManager"):
    ...
    def _update_order_outcomes(self, symbol: str, bar_pct: float, bar_start_ts: int):
        ...
        try:
            ...
            if mgr is not None:
                ...
                cb_cur  = mgr.coinbase_feed.last_prices.get(cb_sym, 0.0)
                cb_base = mgr.coinbase_feed.coinbase_5m_bases.get(cb_sym, 0.0)
                ...
                if Config.BINANCE_ENABLED and mgr.binance_feed is not None:
                    bn_sym = symbol + 'T'
                    bn_cur  = mgr.binance_feed.last_prices.get(bn_sym, 0.0)
                    bn_base = mgr.binance_feed.binance_5m_bases.get(bn_sym, 0.0)
                ...
                cl_cur  = mgr.chainlink_feed.last_prices.get(cl_sym, 0.0)
                cl_base = mgr.chainlink_feed.chainlink_5m_bases.get(cl_sym, 0.0)
...
class BybitManager:
    ...
    def __init__(self, *, chainlink_feed: Any, coinbase_feed: Any, binance_feed: Any):
        ...
        self.bybit_candles: Dict[str, BybitCandle5m] = {
            sym: BybitCandle5m(manager=self) for sym in Config.BYBIT_SYMBOLS
        }
    ...
    # ── Wiring ───────────────────────────────────────────────────────────────────────────
    def attach_components(self, *, rdb: Any, finder: Any, client: Any, price_tracker: Any) -> None:
        """Inject the Redis cache, Polymarket finder, CLOB client, and price tracker.

        Must be called before start_websocket. Each is owned by main.py and the
        order of construction means they aren't available when BybitManager()
        is created; this method binds them in.
        """
        self._rdb = rdb
        self._finder = finder
        self._client = client
    ...
    def attach_validator(self, validator_coro_factory: Any) -> None:
        ...
        if sym in symbol_map:
            ...
            if not aligned:
                ...
                btc_lag = (
                    sym != "BTCUSD"
                    and self._btc_momentum_direction is not None
                    and (timemodule.time() - self._btc_momentum_ts) < Config.BTC_LAG_TTL
                    and ((bybit_5m_pct > 0) == (self._btc_momentum_direction == "UP"))
                )
                ...
                _obi_thresh = self._effective_obi_thresh(sym, btc_lag, gs_direction)
                obi_trend = self.obi_trackers[sym].trend() if sym in self.obi_trackers else 0.0
                obi_contradicts = self._obi_contradicts(obi, obi_trend, _obi_thresh, bybit_5m_pct)
