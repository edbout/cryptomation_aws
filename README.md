...
- Switch raw-signal outcome resolution to Polymarket token mid (was: Bybit bar direction). New `Config.RAW_OUTCOME_SOURCE` env var (`polymarket` default, `bybit` escape hatch). At bar close in `bybit_feed.BybitCandle5m._update_raw_outcomes`, the new `_update_raw_outcomes_pm` looks up each :na record's token via `POLY_MID_CACHE.get`; classifies mid>0.75 as bought-side win, <0.25 as loss, in-between as still-in-flight (left :na for next-bar retry or 6-min expiry). Encoded as `up`/`down` matching Bybit-equivalent semantics so downstream readers (seed_stats_from_raw, dashboards) work unchanged. Emits a 🔍 raw_outcome_compare per-bar log line with agree/disagree/unresolved/missing_mid counts so the PM-vs-Bybit disagreement rate is measurable from logs. Existing records keep their original Bybit-derived outcomes; the change only affects newly-resolved :na records. Revert with `RAW_OUTCOME_SOURCE=bybit`.
- Add independent suppression-outcome resolver (`lib/suppression_store.py::resolve_loop`): background coroutine started from `main.py` that scans `_STORE` every `SUPPRESSED_OUTCOME_SCAN_INTERVAL_SEC` (30s default), and for each entry whose bar closed ≥ `SUPPRESSED_OUTCOME_RESOLVE_DELAY_SEC` (25s default) ago: looks up the (asset, epoch) market via `PolymarketFinder` (Redis-cached, 60s TTL), gets the YES token, fetches its mid via `POLY_MID_CACHE.get` (or `client.get_midpoint` fallback), and emits the standard `🔍 suppressed_outcome | asset | vetoed_dir=… | epoch=… | resolved=YES/NO | would_be=WIN/LOSS` line. Coverage jumps from ~1% of suppressions (pre-existing real-trade-driven path in `order_manager.polymarket_order_outcome`) to ~100% — `pop()` is GIL-atomic so whichever path wins the race emits exactly once, no double-emission. Ambiguous mids (0.25–0.75) retry up to 3 times then log `resolved=NA would_be=NA`. TTL on store entries widened from 600s → 900s to survive retries. Env knobs: `SUPPRESSED_OUTCOME_INDEPENDENT_ENABLED` (default true), `SUPPRESSED_OUTCOME_RESOLVE_DELAY_SEC` (25), `SUPPRESSED_OUTCOME_SCAN_INTERVAL_SEC` (30). Disable with `SUPPRESSED_OUTCOME_INDEPENDENT_ENABLED=false`.
